...
{
    /// <summary>
    /// Handles the request with the ability to invoke next behavior in the pipeline.
    /// </summary>
    /// <param name="request">The incoming request.</param>
    /// <param name="next">The delegate to invoke the next behavior or handler,</param>
    /// <param name="token">Calcellation token.</param>
    /// <returns>The result of the request.</returns>
    Task<TResult> HandleAsync(TRequest request, Func<Task<TResult>> next, CancellationToken token = default);
}